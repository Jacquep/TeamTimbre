  ...
  EVDB.API.call('/events/search', oArgs, function(oData) {
    ...
    for (var i = 0; i < app.events.event.length; i++) {
      if (app.events.event[i].image) {
        ...
        var eventDay = $("<p class='local-event info-event event-date'>").text(
          displayDay
        )
        var eventMonth = $(
          "<p class='local-event info-event event-date'>"
        ).text(displayMonth)
        var eventTime = $("<p class='local-event info-event event-date'>").text(
          displayTime
        )
        var eventTitle = $(
          "<p class='local-event info-event uppercase event-title'>"
        ).text(gData[i].title)
        var eventVenue = $(
          "<p class='local-event info-event event-venue'>"
        ).text(gData[i].venue)
        ...
        eventImage.attr(
          'id',
          gData[i].artist ? gData[i].artist : gData[i].title
        )
